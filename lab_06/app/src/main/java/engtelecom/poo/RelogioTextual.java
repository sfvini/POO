...
public class RelogioTextual extends Relogio implements Cronometro {
    ...
    public RelogioTextual(double x, double y, int tFonte, int modo) throws Exception {
        super(x, y);
        this.modo = modo;
        this.tFonte = (tFonte < 10 || tFonte > 50) ? 24 : tFonte;

        var arquivo = RelogioTextual.class.getClassLoader().getResourceAsStream("JetBrainsMono-Regular.ttf");
        this.fonte = Font.createFont(Font.TRUETYPE_FONT, arquivo);
    ...
    @Override public void setModoRelogioComum() {
        this.modo = 0; this.hora = LocalTime.now().getHour();
        this.minuto = LocalTime.now().getMinute();
        this.segundo = LocalTime.now().getSecond();
    }
    ...
    public void atualizarTempo() {
        if (this.modo == 0 || this.modo == 1) {
            this.segundo++;
            if (this.segundo == 60) { this.segundo = 0; this.minuto++;
                if (this.minuto == 60) { this.minuto = 0; this.hora++;
            ...
            this.segundo--;
            if (this.segundo < 0) { this.segundo = 59; this.minuto--;
                if (this.minuto < 0) { this.minuto = 59; this.hora--; }
            }
        }
    }
